...
def get_effs(wp : str, 
             ...
             abseta : np.ndarray, 
             flav : np.ndarray, 
             cset_eff : CorrectionSet,
             num : np.ndarray) -> tuple[ak.Array, ak.Array, ak.Array]:
...
def get_sf(wp : str,
           ...
           abseta : np.ndarray,
           flav : np.ndarray, 
           cset_sf : CorrectionSet, 
           variation : str, 
           num : np.ndarray) -> ak.Array:
...
def the_single_wp_sf(eff, sf, passWP):
    ...
    PData : Any = ak.where(PData == 0, 1, PData)
...
def single_wp_btagSF(weights,
    ...
    eff_nom, eff_up, eff_dn = get_effs(wp, pt, abseta, flav, cset_eff, num)
    sf_nom, sf_up, sf_dn = get_sfs(wp, pt, abseta, flav, cset_sf, num)

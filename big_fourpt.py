if __name__ == '__main__':
    ...
    datafiles = get_rootfiles('cmseos.fnal.gov','/store/group/lpcpfnano/srothman/Jun06_2024_res4_notransfer_take4/2018/SingleMuon')
    pythiafiles = get_rootfiles('cmseos.fnal.gov', '/store/group/lpcpfnano/srothman/Jun06_2024_res4_notransfer_take4/2018/DYJetsToLL/DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8')
    herwigfiles = get_rootfiles('cmseos.fnal.gov', '/store/group/lpcpfnano/srothman/Jun06_2024_res4_notransfer_take4/2018/DYJetsToLL/DYJetsToLL_M-50_TuneCH3_13TeV-madgraphMLM-herwig7')
    ...
    def doOne(file, onGen=True):
        ...
        if onGen:
            ...
            EEC = readers.rGenEEC.res4
        else:
            ptmask = readers.rRecoJet.jets.pt > 500
            ptmask = ptmask[readers.rRecoEEC.iJet]
            EEC = readers.rRecoEEC.res4

        if ak.sum(ptmask) == 0:
            return np.zeros((4,3,21,21))
    ...
    client = dask.distributed.Client(threads_per_worker=1, n_workers=10)

    print("DOING HERWIG")
    futures = client.map(doOne, herwigfiles)

    actual_ans = np.zeros((4,3,21,21))

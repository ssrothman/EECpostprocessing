...
class MatchingProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        #setup inputs
        jets = reader.jetreader(events, 'updatedJetsPuppi', self.names[0]+"Particles", "Jets")
        ...
        jetMask = np.abs(jets.simonjets.jetEta) < 2.0

        #evtWeight = weights.getEventWeight(events)
        #weight = evtWeight.weight()
        weight = np.ones(len(HLT))

        genjets = reader.jetreader(events, 'GenJet', self.names[0]+"GenParticles", None)
        #genJetMask = (genjets.jets.eta < 99999999) & evtSel.all(*evtSel.names)
        #genJetMask = jetMask
        genJetMask = np.abs(genjets.simonjets.jetEta) < 2.0
